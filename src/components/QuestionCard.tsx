    ...
    <AnimatePresence mode="wait">
      ...
      >
        <div className="flex items-center gap-2">
          <span className="category-label">{t(`${question.id}.category`, { ns: "questions" })}</span>
          {question.optional && (
        ...
        <h2 className="font-display text-2xl font-bold mt-2 mb-1 text-foreground">
          {t(`${question.id}.title`, { ns: "questions" })}
        </h2>
        <p className="text-muted-foreground text-sm mb-5">{t(`${question.id}.prompt`, { ns: "questions" })}</p>
          ...
          <div className="grid grid-cols-1 gap-3">
            {question.options.map((opt) => (
              ...
              >
                ...
                <span className="font-semibold text-foreground">{t(`${question.id}.options.${opt.id}`, { ns: "questions" })}</span>
              </button>
            ))}
          </div>
        )}
            ...
            >
              ...
              {question.options.map((opt) => (
                <option key={opt.id} value={opt.id}>
                  {opt.emoji} {t(`${question.id}.options.${opt.id}`, { ns: "questions" })}
                </option>
              ))}
            </select>
        ...
        {question.type === "text" && (
          <input
            ...
            value={answer}
            onChange={(e) => onAnswer(e.target.value)}
            placeholder={t(`${question.id}.placeholder`, { ns: "questions" })}
